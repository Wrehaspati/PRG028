    ...
    <x-app-layout>
        ...
        <x-slot name="header">
            <h2 class="font-semibold text-xl text-gray-500 leading-tight">
                E-Learning | Edit Teacher | {{ $teacher->id.' - '.$teacher->teacher_name }}
            </h2>
        </x-slot>
                    ...
                    <section class="w-3/5">
                        @if (Session::has('msg')) 
                            <div class="bg-green-500 text-white px-5 py-2 rounded mb-2">
                                <div>
                                    {{ date('d M Y - h:i:s', strtotime($teacher->updated_at)) }}
                                </div> 
                                Perubahan '{{ Session::get('msg') }}' telah berhasil dieksekusi!
                            </div>
                        @else 
                            <label for="" class="font-bold">Perubahan terakhir pada {{ date('d M Y - h:i:s', strtotime($teacher->updated_at)) }}</label>
                        @endif
                        <form action="{{ route('teacher.update', $teacher->id) }}" method="POST" class="w-full border border-gray-400 rounded p-5">
                            @csrf
                            @method('PUT')
                            <div class="text-lg font-bold">
                                <label for="id">ID Guru</label>
                                <input class="w-full border border-gray-400 rounded" type="text" name="id" required
                                    value="{{ $teacher->id }}" id="id">
                            </div>
                            <div class="text-lg font-bold mt-5">
                                <label for="teacher_name">Nama Guru</label>
                                <input class="w-full border border-gray-400 rounded" type="text" name="teacher_name" required
                                    value="{{ $teacher->teacher_name }}" id="teacher_name">
                            </div>
                            ...
                            <div class="text-lg font-bold mt-5">
                                ...
                                <input class="w-full border border-gray-400 rounded" type="text" name="token" required
                                    value="{{ $teacher->token }}" id="token">
                            </div>
