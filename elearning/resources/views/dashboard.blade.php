                    ...
                    @if($courses != null || $courses != "")
                        @foreach ($courses as $subject) 
                            @php
                                ...
                                $color = $arr[rand(0,16)];
                            @endphp
                            <x-grade-card :$grade :$subject class="pb-4" style="background-color: {{ $color }}" />
                        @endforeach
                    @else
                        <h1 class="text-[2rem] font-bold pt-4">Belum ada kelas?! Harap Hubungi Guru / Administrator</h1 class="font-lg" >
                    @endif
                </div>
            </section>
        </div>
